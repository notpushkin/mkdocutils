...
from mkdocutils import plugins, theme, utils
from mkdocutils.config.base import Config, ValidationError
    ...
    def run_validation(self, value):
        ...
        if not isinstance(value, Sequence):
            raise ValidationError(
                'Expected a sequence of mappings, but a %s '
...
class FilesystemObject(Type):
    ...
    def pre_validation(self, config, key_name):
        ...
        ) if config.config_file_path else None
    ...
    def run_validation(self, value):
        ...
        if self.exists and not self.existence_test(value):
            raise ValidationError(
                "The path {path} isn't an existing {name}.".
...
class Theme(BaseConfigOption):
    ...
    def validate(self, value):
        ...
        raise ValidationError(
            'Invalid type "{}". Expected a string or key/value pairs.'.format(type(value)),
        )
...
class Nav(OptionallyRequired):
    ...
    def post_validation(self, config, key_name):
        ...
        if key_name == 'pages' and config['pages'] is not None:
            ...
            warning = (
                "The 'pages' configuration option has been deprecated and will "
