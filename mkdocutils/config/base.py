...
class Config(UserDict):
        ...
        if config_file_path is not None and not isinstance(config_file_path, str):
            try:
                ...
                raise ValidationError(
                    "config_file_path is not a Unicode string.",
                )
    ...
    def load_file(self, config_file):
        try:
            ...
            raise exceptions.ConfigurationError(
                "MkDocs encountered an error parsing the configuration file: {}".format(
                    e,
                ),
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('mkdocutils.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `mkdocutils.yml` filename will loaded.
