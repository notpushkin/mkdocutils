...
from mkdocutils.commands.build import build
from mkdocutils.config import load_config
...
def serve(
    config_file=None, dev_addr=None, strict=None, theme=None,
    ...
    try:
        ...
        if livereload in ['livereload', 'dirty']:
            _livereload(
                host, port, config, builder,
                site_dir, watch_theme, delay=wait,
            )
        else:
