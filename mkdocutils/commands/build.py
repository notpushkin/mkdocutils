...
def get_context(nav, files, config, page=None, base_url=''):
    ...
    extra_javascript = utils.create_media_urls(
        config['extra_javascript'], page, base_url,
    )
...
def _build_theme_template(template_name, env, files, config, nav):
    ...
    try:
        ...
        log.warning(
            "Template skipped: '{}' not found in theme directories.".format(template_name),
        )
        return
    ...
    if output.strip():
        ...
        log.info(
            "Template skipped: '{}' generated empty output.".format(template_name),
        )
...
def _build_extra_template(template_name, files, config, nav):
    ...
    if file is None:
        log.warning(
            "Template skipped: '{}' not found in docs_dir.".format(template_name),
        )
        return
...
def _build_page(page, config, doc_files, nav, env, dirty=False):
    ...
    try:
        ...
        if output.strip():
            ...
            log.info(
                "Page skipped: '{}'. Generated empty output.".format(
                page.file.src_path,
                ),
            )
