...
from mkdocutils import config, utils
...
def tempdir(files=None, **kw):
    ...
    """
    files = {f: '' for f in files} if isinstance(
        files, (list, tuple),
    ) else files or {}
...
class PathAssertionMixin:
    ...
    def assertPathExists(self, *parts):
        ...
        if not os.path.exists(path):
            msg = self._formatMessage(
                None, "The path '{}' does not exist".format(path),
            )
    ...
    def assertPathNotExists(self, *parts):
        ...
        if os.path.exists(path):
            msg = self._formatMessage(
                None, "The path '{}' does exist".format(path),
            )
            raise self.failureException(msg)
    ...
    def assertPathIsFile(self, *parts):
        ...
        if not os.path.isfile(path):
            msg = self._formatMessage(
                None, "The path '{}' is not a file that exists".format(path),
            )
            raise self.failureException(msg)
    ...
    def assertPathNotFile(self, *parts):
        ...
        if os.path.isfile(path):
            msg = self._formatMessage(
                None, "The path '{}' is a file that exists".format(path),
            )
            raise self.failureException(msg)
    ...
    def assertPathIsDir(self, *parts):
        ...
        if not os.path.isdir(path):
            msg = self._formatMessage(
                None, "The path '{}' is not a directory that exists".format(path),
            )
            raise self.failureException(msg)
    ...
    def assertPathNotDir(self, *parts):
        ...
        if os.path.isfile(path):
            msg = self._formatMessage(
                None, "The path '{}' is a directory that exists".format(path),
            )
            raise self.failureException(msg)
