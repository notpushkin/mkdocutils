...
from mkdocutils.config.config_options import ValidationError
from mkdocutils.contrib import search
from mkdocutils.contrib.search import search_index
from mkdocutils.structure.files import File
from mkdocutils.structure.pages import Page
from mkdocutils.structure.toc import get_toc
from mkdocutils.tests.base import dedent, get_markdown_toc, load_config
...
class SearchPluginTests(unittest.TestCase):
    ...
    def test_event_on_config_include_search_page(self):
        ...
        self.assertEqual(
            result['theme'].static_templates, {
            '404.html', 'sitemap.xml', 'search.html',
            },
        )
        self.assertEqual(len(result['theme'].dirs), 3)
        self.assertEqual(result['extra_javascript'], ['search/main.js'])
    ...
    def test_event_on_post_build_search_index_only(self, mock_copy_file, mock_write_file):
        ...
        plugin.load_config({'lang': ['es']})
        config = load_config(
            theme={'name': 'mkdocs', 'search_index_only': True},
        )
        plugin.on_pre_build(config)
        plugin.on_post_build(config)
        self.assertEqual(mock_copy_file.call_count, 0)
...
class SearchIndexTests(unittest.TestCase):
    ...
    def test_create_search_index(self):
        ...
        full_content = ''.join(
            """Heading{0}Content{0}""".format(i) for i in range(1, 4)
        )
        ...
        for page in pages:
            ...
            self.assertEqual(
                index._entries[1]['location'], "{}#heading-1".format(loc),
            )

            self.assertEqual(index._entries[2]['title'], "Heading 2")
            self.assertEqual(
                strip_whitespace(
                index._entries[2]['text'],
                ), "Content2",
            )
            self.assertEqual(
                index._entries[2]['location'], "{}#heading-2".format(loc),
            )

            self.assertEqual(index._entries[3]['title'], "Heading 3")
            self.assertEqual(
                strip_whitespace(
                index._entries[3]['text'],
                ), "Content3",
            )
            self.assertEqual(
