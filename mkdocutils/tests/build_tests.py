...
from mkdocutils.commands import build
from mkdocutils.structure.files import File, Files
from mkdocutils.structure.nav import get_navigation
from mkdocutils.structure.pages import Page
from mkdocutils.tests.base import PathAssertionMixin, load_config, tempdir
from mkdocutils.utils import meta
...
class BuildTests(PathAssertionMixin, unittest.TestCase):
    ...
    def test_context_base_url_relative_no_page(self):
        ...
        context = build.get_context(
            mock.Mock(), mock.Mock(), cfg, base_url='..',
        )
        self.assertEqual(context['base_url'], '..')
    ...
    def test_context_base_url_absolute_nested_no_page(self):
        cfg = load_config(use_directory_urls=False)
        context = build.get_context(
            mock.Mock(), mock.Mock(), cfg, base_url='/foo/',
        )
    ...
    def test_build_theme_template(self, mock_build_template, mock_write_file):
        ...
        build._build_theme_template(
            'main.html', env, mock.Mock(), cfg, mock.Mock(),
        )
    ...
    def test_build_sitemap_template(self, mock_gzip_gzipfile, mock_build_template, mock_write_file):
        ...
        env = cfg['theme'].get_env()
        build._build_theme_template(
            'sitemap.xml', env, mock.Mock(), cfg, mock.Mock(),
        )
        self.assert_mock_called_once(mock_write_file)
        self.assert_mock_called_once(mock_build_template)
    ...
    def test_skip_missing_extra_template(self):
        ...
        with self.assertLogs('mkdocs', level='INFO') as cm:
            build._build_extra_template(
                'missing.html', files, cfg, mock.Mock(),
            )
        self.assertEqual(
            cm.output,
    ...
    def test_populate_page_read_error(self, docs_dir, mock_open):
        cfg = load_config(docs_dir=docs_dir)
        file = File(
            'missing.md', cfg['docs_dir'],
            cfg['site_dir'], cfg['use_directory_urls'],
        )
        page = Page('Foo', file, cfg)
        with self.assertLogs('mkdocs', level='ERROR') as cm:
            self.assertRaises(
                OSError, build._populate_page,
                page, cfg, Files([file]),
            )
        self.assertEqual(
            cm.output, [
    ...
    def test_build_page_error(self, site_dir, mock_write_file):
        ...
        with self.assertLogs('mkdocs', level='ERROR') as cm:
            self.assertRaises(
                OSError, build._build_page, page,
                cfg, files, nav, cfg['theme'].get_env(),
            )
        self.assertEqual(
            cm.output,
